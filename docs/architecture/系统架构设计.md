...
项目当前不是 Git 仓库，未发现 `.gitmodules`，按单模块项目处理。源码目录为 `Sources/LateStudySimulator`，资源目录包括 `Resources/AudioCues` 和 `Resources/AudioLoops`，构建配置将两类音频资源复制进包资源。来源：`Package.swift:14`、`Package.swift:16`、`Package.swift:18`、`Package.swift:19`。
...
*待确认*：README 提到 `dist/晚自习模拟器.app`、DMG 和 zip 已生成，但当前源码目录扫描未发现 `dist/`。来源：`README.md:17`。
...
| 非 Git 仓库 | 当前目录无 `.git`，无法通过 Git 历史追踪初始化变更。 | `git status --short` 返回非仓库 |
