...
| 参与者 | 功能角色 | 依据 |
| --- | --- | --- |
| 玩家/学生 | 在座位中观察、行动、维持心理能量与任务进度；获准离座后可短时进入教室/走廊自由活动。 | `GameModels.swift`、`GameManager.swift`、`README.md` |
...
玩家视角包括前方、低头、抬头、左侧、右侧和后方；鼠标拖拽控制学生视角，坐着回头会显著增加暴露和压力。获准离座后进入 60 秒自由活动，按住空格沿当前朝向前进，按住 Shift 侧身通过窄缝。来源：`GameModels.swift`、`GameManager.swift`、`ClassroomSceneView.swift`。

学生相机在座位状态下映射头部视角，在自由活动状态下映射玩家位置和朝向；轻量碰撞系统限制桌椅、墙体、窗户和走廊柜子。`ClassroomCoordinator` 接管鼠标拖拽、空格前进和 Shift 侧身输入。来源：`GameManager.swift`、`ClassroomSceneView.swift`。
...
3D 场景由 `ClassroomCoordinator` 构建，包含教室、家具、灯光、玩家桌面作业/笔/手部、老师、同学、墙钟、黑板、透明窗户、左侧户外、右侧贯通走廊、前后班级门牌、洗手间和持续旋转吊扇。来源：`ClassroomSceneView.swift`。
