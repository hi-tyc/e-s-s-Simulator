...
enum PrologueBeatID: String, Codable, CaseIterable {
    ...
    var tutorialInstruction: String {
        ...
        case .lookDownHall:
#if os(iOS)
            return "现在让我们熟悉视角。直接在画面上拖动即可转动视角，双击画面可以回到前方。请环视周围，累计移动视角 5 秒；视角不动时计时会暂停。"
#else
            return "现在让我们熟悉如何控制视角。移动鼠标即可控制转动视角，按下 ～ 键即可释放鼠标，再按一次恢复控制。你的鼠标现在是释放状态，需要先按 ～ 获取控制。请环视周围，累计移动视角 5 秒；视角不动时计时会暂停。此步骤不能用 C 跳过。"
#endif
        case .returnToSeat:
#if os(iOS)
            return "使用左侧摇杆移动，在画面上拖动控制方向。侧身可以通过更窄的空间，奔跑速度更快但会增加饥饿。请走回第三排座位，靠近后点击右侧入座按钮。"
#else
        ...
        case .placeWater:
#if os(iOS)
            return "点击“放好水杯”完成操作。靠近门、储物柜、饮水机、厕所和座位时，手机右侧都会出现对应的交互按钮。"
#else
            return "先把水杯放到桌上，做好晚自习的一切准备。单击 E 键即可完成操作，游戏里的其他交互也都是 E 键。放好后会解锁 C，可以提前结束剩余倒计时。"
#endif
        case .studyHallRhythm:
#if os(iOS)
            return "留在座位观察教室。你仍可在画面上拖动环顾四周，双击回到前方。"
#else
        ...
        case .accessibility:
#if os(iOS)
            return "你可以调整字幕、声音和动态效果。点击“调整体验”即可；之后也可以随时点击右上角齿轮再次打开。"
#else
    ...
    var hint: String? {
        ...
        case .returnToSeat:
#if os(iOS)
            return "使用左侧摇杆，靠近后点击入座"
#else
            return "使用 WASD，或等待苏念自己走回去"
#endif
        case .placeWater:
#if os(iOS)
            return "点击“放好水杯”"
