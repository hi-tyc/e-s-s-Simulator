...
final class GameManager: ObservableObject {
    ...
    func beginInitialGameGuideIfNeeded() {
        ...
        menuGuideCountdown = 5
        menuGuideTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
            Task { @MainActor in self?.advanceMenuGuideCountdown() }
        }
    }
    ...
    func presentGameGuide(isInitialPresentation: Bool = false) {
        ...
        gameGuideExitCountdown = isInitialPresentation && hasCompletedInitialGameGuide == false ? 10 : 0
    ...
    func startGame(as role: PlayableRole = .regularStudent) {
        stopPrologueTimer()
    ...
    func updatePrologueDwell(delta: TimeInterval) {
        ...
        if matches && prologueDwell >= 1.0 {
            ...
            message = prologueCompletionMessage("你已经认真看见了林澈的状态。")
        }
    }
    ...
    func completeCurrentPrologueEarly() {
        ...
        completePrologueBeat(prologueCurrentBeat, source: .fallback)
    }
    ...
    func confirmPrologueInteraction() {
        ...
        case .placeWater:
            prologueActionReady = true
            message = prologueCompletionMessage("水杯已经放在手边。")
        case .accessibility:
            acknowledgeAccessibilityTutorial(openSettings: false)
        case .settleBreath:
            prologueActionReady = true
            message = prologueCompletionMessage("你完成了一次自我调整。")
        case .returnToSeat:
            confirmPrologueSeat()
        default:
            break
    ...
    func confirmPrologueSeat() -> Bool {
        ...
        cameraPose = .forward
        prologueActionReady = true
        message = prologueCompletionMessage("你已经坐回自己的位置。")
        return true
    }
    ...
    func acknowledgeSettleBreathWithoutAction() {
        ...
        prologueActionReady = true
        message = prologueCompletionMessage("你决定先安静地坐一会儿。")
    }
    ...
    func acknowledgeAccessibilityTutorial(openSettings: Bool) {
        ...
        if openSettings {
            ...
            prologueActionReady = true
            message = prologueCompletionMessage("你已经确认了辅助设置。")
        }
    }
    ...
    func closeAccessibilityPanel() {
        ...
        if isPrologueActive, prologueState.currentBeat == .accessibility {
            prologueState.accessibilityTutorialAcknowledged = true
            prologueActionReady = true
            message = prologueCompletionMessage("辅助设置已确认。")
        }
    }
    ...
    private func prologueCompletionMessage(_ prefix: String) -> String {
#if os(iOS)
        return "\(prefix) 点击“完成本步”可以提前继续，也可以留在这里片刻。"
#else
        return "\(prefix) 按 C 可以提前结束这一步，也可以继续留在这里。"
#endif
    }

    private func freeRoamControlMessage(duration: TimeInterval) -> String {
#if os(iOS)
        return "老师点头同意。你获得 \(Int(duration)) 秒离座活动时间。使用左侧摇杆行走，在画面上拖动调整方向；靠近可交互物体时，右侧会出现操作按钮。"
#else
        return "老师点头同意。你获得 \(Int(duration)) 秒离座活动时间，移动鼠标调整方向，使用 WASD 行走；靠近前后门时可以开门或关门。"
#endif
    }

    private var seatInteractionMessage: String {
#if os(iOS)
        return "你已经靠近自己的座位。点击右侧“坐回座位”。"
#else
        return "你已经靠近自己的座位。按 E 入座。"
#endif
    }
    ...
    private func beginStudentFreeRoam(duration: TimeInterval = 60, openingMessage: String? = nil) {
        ...
        player.posture = .standing
        gameState = .playing
        message = openingMessage ?? freeRoamControlMessage(duration: duration)
    ...
    func moveStudentFreeRoam(forward: Double, strafe: Double, deltaTime: Double) {
        ...
        if isPrologueActive, prologueState.currentBeat == .returnToSeat {
            ...
            if prologueSeatNearby && wasNearby == false {
                message = seatInteractionMessage
            }
        }
    ...
    func execute(_ action: PlayerAction) {
        ...
        if isPrologueActive {
            if prologueState.currentBeat == .settleBreath, action == .breathe {
                ...
                prologueActionReady = true
                message = prologueCompletionMessage("呼吸慢了下来。")
            }
            return
        }
