...
typealias SceneFont = NSFont
typealias SceneScalar = CGFloat
#else
import UIKit
typealias SceneColor = UIColor
typealias SceneFont = UIFont
typealias SceneScalar = Float
#endif
import CoreGraphics
import SceneKit
import SwiftUI
...
private extension UIColor {
    ...
    func blended(withFraction fraction: CGFloat, of color: UIColor) -> UIColor? {
        ...
        guard getRed(&r1, green: &g1, blue: &b1, alpha: &a1), color.getRed(&r2, green: &g2, blue: &b2, alpha: &a2) else { return nil }
...
struct ClassroomSceneView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> SCNView {
        ...
        view.backgroundColor = SceneColor(calibratedRed: 0.04, green: 0.045, blue: 0.05, alpha: 1)
...
#else
@MainActor
struct ClassroomSceneView: UIViewRepresentable {
    @ObservedObject var game: GameManager

    func makeCoordinator() -> ClassroomCoordinator { ClassroomCoordinator() }

    func makeUIView(context: Context) -> SCNView {
        let view = SCNView()
        let isPhone = UIDevice.current.userInterfaceIdiom == .phone
        let useReducedRendering = isPhone || ProcessInfo.processInfo.isLowPowerModeEnabled
        view.scene = context.coordinator.scene
        view.pointOfView = context.coordinator.cameraRig
        view.backgroundColor = .black
        view.preferredFramesPerSecond = useReducedRendering ? 30 : 60
        view.antialiasingMode = useReducedRendering ? .none : .multisampling2X
        view.rendersContinuously = false
        if isPhone {
            // A 2x drawable avoids the 3x pixel cost on Pro iPhones while preserving UI sharpness.
            view.contentScaleFactor = 2
        }
        let pan = UIPanGestureRecognizer(target: context.coordinator, action: #selector(ClassroomCoordinator.handleIOSPan(_:)))
        view.addGestureRecognizer(pan)
        let pinch = UIPinchGestureRecognizer(target: context.coordinator, action: #selector(ClassroomCoordinator.handleIOSPinch(_:)))
        view.addGestureRecognizer(pinch)
        let tap = UITapGestureRecognizer(target: context.coordinator, action: #selector(ClassroomCoordinator.handleIOSDoubleTap(_:)))
    ...
    static func dismantleUIView(_ view: SCNView, coordinator: ClassroomCoordinator) {
        coordinator.teardownInput()
    }
}
#endif
...
final class ClassroomCoordinator {
    ...
    @objc func handleIOSPan(_ gesture: UIPanGestureRecognizer) {
        let translation = gesture.translation(in: gesture.view)
        let velocity = gesture.velocity(in: gesture.view)
        let speed = hypot(velocity.x, velocity.y)
        let isPhone = UIDevice.current.userInterfaceIdiom == .phone
        let baseSensitivity: CGFloat = isPhone ? 1.55 : 1.05
        let acceleration = min(1.65, 1 + max(0, speed - 350) / 2_500)
        let dx = Double(translation.x * baseSensitivity * acceleration)
        let dy = Double(translation.y * baseSensitivity * 0.72 * acceleration)
        gesture.setTranslation(.zero, in: gesture.view)
        guard let game = currentGame, game.viewMode == .student else { return }
        game.rotateStudentView(deltaX: dx, deltaY: dy)
    }

    @objc func handleIOSPinch(_ gesture: UIPinchGestureRecognizer) {
        guard let camera = cameraRig.camera else { return }
        if gesture.state == .began { camera.fieldOfView = 78 }
        camera.fieldOfView = min(92, max(58, camera.fieldOfView / gesture.scale))
        gesture.scale = 1
    }

    @objc func handleIOSDoubleTap(_ gesture: UITapGestureRecognizer) {
    ...
    func update(game: GameManager) {
        ...
        scene.background.contents = game.isPrologueActive && game.prologueCurrentBeat == .gateArrival
            ? SceneColor(calibratedRed: 0.16, green: 0.2, blue: 0.27, alpha: 1)
            : SceneColor(calibratedRed: 0.04, green: 0.045, blue: 0.05, alpha: 1)
#if os(macOS)
    ...
    private func makePrologueExterior() -> SCNNode {
        ...
        ground.geometry?.firstMaterial?.diffuse.contents = SceneColor(calibratedRed: 0.18, green: 0.22, blue: 0.24, alpha: 1)
        ...
        building.geometry?.firstMaterial?.diffuse.contents = SceneColor(calibratedRed: 0.48, green: 0.5, blue: 0.48, alpha: 1)
        root.addChildNode(building)

        let signGeometry = SCNText(string: "教学楼", extrusionDepth: 0.015)
        signGeometry.font = SceneFont.systemFont(ofSize: 0.42, weight: .semibold)
        signGeometry.firstMaterial?.diffuse.contents = SceneColor(calibratedWhite: 0.86, alpha: 1)
        ...
        for floor in 0..<3 {
            for column in 0..<6 {
                ...
                window.geometry?.firstMaterial?.diffuse.contents = SceneColor(calibratedRed: 0.96, green: 0.82, blue: 0.5, alpha: 1)
                window.geometry?.firstMaterial?.emission.contents = SceneColor(calibratedRed: 0.28, green: 0.2, blue: 0.08, alpha: 1)
        ...
        gateLeft.geometry?.firstMaterial?.diffuse.contents = SceneColor.darkGray
        ...
        let dusk = SCNLight()
        dusk.type = .omni
        dusk.color = SceneColor(calibratedRed: 1, green: 0.58, blue: 0.32, alpha: 1)
        dusk.intensity = 760
    ...
    private func makePrologueStudentSilhouette(index: Int) -> SCNNode {
        ...
        body.geometry?.firstMaterial?.diffuse.contents = index.isMultiple(of: 2)
            ? SceneColor(calibratedRed: 0.16, green: 0.22, blue: 0.35, alpha: 1)
            : SceneColor(calibratedRed: 0.28, green: 0.2, blue: 0.25, alpha: 1)
        root.addChildNode(body)
        let head = SCNNode(geometry: SCNSphere(radius: 0.15))
        head.position.y = 1.15
        head.geometry?.firstMaterial?.diffuse.contents = SceneColor(calibratedRed: 0.55, green: 0.4, blue: 0.3, alpha: 1)
    ...
    private func makePrologueTree(index: Int) -> SCNNode {
        ...
        trunk.geometry?.firstMaterial?.diffuse.contents = SceneColor(calibratedRed: 0.2, green: 0.13, blue: 0.09, alpha: 1)
        ...
        canopy.geometry?.firstMaterial?.diffuse.contents = index.isMultiple(of: 2)
            ? SceneColor(calibratedRed: 0.12, green: 0.28, blue: 0.2, alpha: 1)
            : SceneColor(calibratedRed: 0.18, green: 0.34, blue: 0.22, alpha: 1)
    ...
    private func makeEnvironment() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 8, height: 0.04, length: 12, color: SceneColor(calibratedRed: 0.48, green: 0.38, blue: 0.28, alpha: 1), position: SCNVector3(0, -0.02, 0)))
        ...
        root.addChildNode(box(width: 8, height: 0.04, length: 12, color: SceneColor(calibratedWhite: 0.93, alpha: 1), position: SCNVector3(0, 3.52, 0)))
        root.addChildNode(makeDoors())

        root.addChildNode(box(width: 4.2, height: 1.2, length: 0.05, color: SceneColor(calibratedRed: 0.06, green: 0.16, blue: 0.09, alpha: 1), position: SCNVector3(0, 1.95, -5.94)))
        root.addChildNode(box(width: 1.9, height: 0.9, length: 0.05, color: SceneColor(calibratedRed: 0.64, green: 0.48, blue: 0.31, alpha: 1), position: SCNVector3(0, 0.55, -4.7)))
    ...
    private func makeCorridor() -> SCNNode {
        let root = SCNNode()
        let floorColor = SceneColor(calibratedRed: 0.34, green: 0.35, blue: 0.36, alpha: 1)
        let wall = SceneColor(calibratedRed: 0.72, green: 0.74, blue: 0.7, alpha: 1)
        let dimWall = SceneColor(calibratedRed: 0.52, green: 0.55, blue: 0.55, alpha: 1)

        root.addChildNode(box(width: 2.8, height: 0.035, length: 20.8, color: floorColor, position: SCNVector3(5.25, -0.018, -1.8)))
        root.addChildNode(box(width: 2.8, height: 0.035, length: 20.8, color: SceneColor(calibratedWhite: 0.82, alpha: 1), position: SCNVector3(5.25, 2.96, -1.8)))
        ...
        for z in [-5.2, -4.55, -3.25] {
            root.addChildNode(box(width: 0.42, height: 1.05, length: 0.36, color: SceneColor(calibratedRed: 0.18, green: 0.28, blue: 0.42, alpha: 1), position: SCNVector3(6.55, 0.62, Float(z))))
            root.addChildNode(box(width: 0.03, height: 0.92, length: 0.28, color: SceneColor(calibratedRed: 0.08, green: 0.13, blue: 0.2, alpha: 1), position: SCNVector3(6.32, 0.63, Float(z))))
        }
        ...
        for z in [-0.9, -0.2, 0.5, 1.2, 1.9, 2.6] {
            root.addChildNode(box(width: 0.36, height: 0.95, length: 0.32, color: SceneColor(calibratedRed: 0.19, green: 0.27, blue: 0.36, alpha: 1), position: SCNVector3(6.55, 0.58, Float(z))))
            root.addChildNode(box(width: 0.024, height: 0.82, length: 0.24, color: SceneColor(calibratedRed: 0.08, green: 0.12, blue: 0.18, alpha: 1), position: SCNVector3(6.32, 0.58, Float(z))))
        }

        let corridorLight = SCNLight()
        corridorLight.type = .omni
        corridorLight.intensity = 210
        corridorLight.color = SceneColor(calibratedRed: 0.86, green: 0.92, blue: 1.0, alpha: 1)
        let lightNode = SCNNode()
        lightNode.light = corridorLight
        lightNode.position = SCNVector3(5.15, 2.72, -4.35)
        root.addChildNode(lightNode)
        root.addChildNode(box(width: 0.9, height: 0.02, length: 0.18, color: SceneColor(calibratedWhite: 0.92, alpha: 1), position: SCNVector3(5.15, 2.9, -4.35)))
        for z in [-0.6, 1.35, 2.85] {
            let windowLight = SCNLight()
            windowLight.type = .omni
            windowLight.intensity = 120
            windowLight.color = SceneColor(calibratedRed: 0.86, green: 0.92, blue: 1.0, alpha: 1)
            ...
            root.addChildNode(box(width: 0.72, height: 0.018, length: 0.14, color: SceneColor(calibratedWhite: 0.92, alpha: 1), position: SCNVector3(5.15, 2.88, Float(z))))
        }
        ...
        root.addChildNode(makeText("走廊", size: 0.09, color: SceneColor(calibratedWhite: 0.12, alpha: 1), position: SCNVector3(4.72, 1.75, -5.9)))
        return root
    }

    private func makeCorridorWallDetails() -> SCNNode {
        let root = SCNNode()
        let baseColor = SceneColor(calibratedRed: 0.46, green: 0.49, blue: 0.47, alpha: 1)
        ...
        root.addChildNode(box(width: 2.65, height: 0.38, length: 0.04, color: baseColor, position: SCNVector3(5.25, 0.2, 8.39)))
        let seam = SceneColor(calibratedRed: 0.25, green: 0.27, blue: 0.27, alpha: 1)
    ...
    private func makeCorridorDoor(label: String, z: Float) -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.05, height: 1.7, length: 0.78, color: SceneColor(calibratedRed: 0.42, green: 0.28, blue: 0.16, alpha: 1), position: SCNVector3(4.11, 0.92, z)))
        root.addChildNode(box(width: 0.055, height: 0.22, length: 0.72, color: SceneColor(calibratedWhite: 0.88, alpha: 1), position: SCNVector3(4.08, 1.92, z)))
        root.addChildNode(makePlaqueText(label, position: SCNVector3(4.145, 1.9, z)))
        root.addChildNode(sphere(radius: 0.026, color: SceneColor(calibratedRed: 0.86, green: 0.68, blue: 0.28, alpha: 1), position: SCNVector3(4.05, 0.98, z + 0.25)))
        return root
    }
    ...
    private func makeCurrentClassPlaque(z: Float) -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.055, height: 0.24, length: 0.82, color: SceneColor(calibratedWhite: 0.92, alpha: 1), position: SCNVector3(4.1, 2.2, z)))
        root.addChildNode(box(width: 0.006, height: 0.12, length: 0.54, color: SceneColor(calibratedWhite: 0.82, alpha: 1), position: SCNVector3(4.142, 2.18, z)))
    ...
    private func makePlaqueText(_ string: String, position: SCNVector3) -> SCNNode {
        ...
        text.font = SceneFont.systemFont(ofSize: 0.16, weight: .bold)
        text.flatness = 0.2
        text.firstMaterial = material(SceneColor(calibratedWhite: 0.02, alpha: 1))
        ...
        textNode.eulerAngles = SCNVector3(0, SceneScalar.pi / 2, 0)
    ...
    private func makePlayerLocker() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.42, height: 1.05, length: 0.36, color: SceneColor(calibratedRed: 0.14, green: 0.34, blue: 0.5, alpha: 1), position: SCNVector3(6.55, 0.62, -3.9)))
        root.addChildNode(box(width: 0.032, height: 0.92, length: 0.28, color: SceneColor(calibratedRed: 0.06, green: 0.12, blue: 0.18, alpha: 1), position: SCNVector3(6.315, 0.63, -3.9)))
        playerLockerDoorNode.addChildNode(box(width: 0.026, height: 0.86, length: 0.26, color: SceneColor(calibratedRed: 0.18, green: 0.44, blue: 0.66, alpha: 1), position: SCNVector3Zero))
        playerLockerDoorNode.addChildNode(sphere(radius: 0.018, color: SceneColor(calibratedRed: 0.88, green: 0.68, blue: 0.28, alpha: 1), position: SCNVector3(-0.018, 0.08, 0.09)))
        root.addChildNode(playerLockerDoorNode)
        updatePlayerLocker(isOpen: false)
        let label = makeText("我的柜", size: 0.04, color: SceneColor(calibratedWhite: 0.96, alpha: 1), position: SCNVector3(6.275, 1.18, -4.02))
        label.eulerAngles.y = -SceneScalar.pi / 2
    ...
    private func makeNeighborWindow(z: Float) -> SCNNode {
        ...
        root.addChildNode(box(width: 0.055, height: 0.9, length: 0.035, color: SceneColor(calibratedWhite: 0.76, alpha: 1), position: SCNVector3(4.075, 1.55, z - 0.31)))
        root.addChildNode(box(width: 0.055, height: 0.9, length: 0.035, color: SceneColor(calibratedWhite: 0.76, alpha: 1), position: SCNVector3(4.075, 1.55, z + 0.31)))
        root.addChildNode(box(width: 0.055, height: 0.035, length: 0.66, color: SceneColor(calibratedWhite: 0.76, alpha: 1), position: SCNVector3(4.075, 1.98, z)))
        root.addChildNode(box(width: 0.055, height: 0.035, length: 0.66, color: SceneColor(calibratedWhite: 0.76, alpha: 1), position: SCNVector3(4.075, 1.12, z)))
        return root
    }

    private func makeNeighborRoomInterior(z: Float) -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 2.1, height: 0.035, length: 3.7, color: SceneColor(calibratedRed: 0.48, green: 0.38, blue: 0.28, alpha: 1), position: SCNVector3(3.0, -0.01, z)))
        root.addChildNode(box(width: 0.035, height: 2.2, length: 3.7, color: SceneColor(calibratedRed: 0.68, green: 0.69, blue: 0.64, alpha: 1), position: SCNVector3(3.78, 1.08, z)))
        root.addChildNode(box(width: 2.1, height: 2.2, length: 0.035, color: SceneColor(calibratedRed: 0.66, green: 0.67, blue: 0.62, alpha: 1), position: SCNVector3(2.9, 1.08, z - 1.84)))
        root.addChildNode(box(width: 2.1, height: 2.2, length: 0.035, color: SceneColor(calibratedRed: 0.66, green: 0.67, blue: 0.62, alpha: 1), position: SCNVector3(2.9, 1.08, z + 1.84)))
        for row in 0..<2 {
            for column in 0..<3 {
                let deskX = 2.52 + Float(column) * 0.48
                let deskZ = z - 0.95 + Float(row) * 0.78
                root.addChildNode(box(width: 0.34, height: 0.055, length: 0.22, color: SceneColor(calibratedRed: 0.58, green: 0.44, blue: 0.3, alpha: 1), position: SCNVector3(deskX, 0.78, deskZ)))
                root.addChildNode(box(width: 0.12, height: 0.22, length: 0.06, color: SceneColor(calibratedRed: 0.22, green: 0.26, blue: 0.44, alpha: 1), position: SCNVector3(deskX + 0.02, 0.96, deskZ + 0.08)))
            }
        }
        root.addChildNode(box(width: 0.036, height: 0.52, length: 1.15, color: SceneColor(calibratedRed: 0.06, green: 0.14, blue: 0.08, alpha: 1), position: SCNVector3(3.73, 1.72, z - 1.32)))
        root.addChildNode(box(width: 0.5, height: 0.72, length: 0.035, color: SceneColor(calibratedRed: 0.76, green: 0.74, blue: 0.68, alpha: 1), position: SCNVector3(2.85, 1.56, z + 1.82)))
        return root
    }

    private func makeOutdoorConnector() -> SCNNode {
        let root = SCNNode()
        let floor = SceneColor(calibratedRed: 0.36, green: 0.37, blue: 0.36, alpha: 1)
        let wall = SceneColor(calibratedRed: 0.69, green: 0.71, blue: 0.67, alpha: 1)
        let frame = SceneColor(calibratedWhite: 0.72, alpha: 1)

        root.addChildNode(box(width: 1.85, height: 0.035, length: 2.2, color: floor, position: SCNVector3(3.12, -0.018, -9.35)))
        root.addChildNode(box(width: 1.85, height: 0.035, length: 2.2, color: SceneColor(calibratedWhite: 0.78, alpha: 1), position: SCNVector3(3.12, 2.92, -9.35)))
        ...
        root.addChildNode(box(width: 0.035, height: 1.9, length: 1.75, color: SceneColor(calibratedRed: 0.48, green: 0.72, blue: 0.96, alpha: 1), position: SCNVector3(1.95, 1.43, -9.35)))
        root.addChildNode(box(width: 0.035, height: 0.34, length: 1.1, color: SceneColor(calibratedRed: 0.24, green: 0.42, blue: 0.22, alpha: 1), position: SCNVector3(1.92, 0.35, -9.45)))
        root.addChildNode(box(width: 0.035, height: 0.9, length: 0.12, color: SceneColor(calibratedRed: 0.22, green: 0.42, blue: 0.22, alpha: 1), position: SCNVector3(1.91, 0.8, -8.85)))
        root.addChildNode(sphere(radius: 0.26, color: SceneColor(calibratedRed: 0.16, green: 0.48, blue: 0.2, alpha: 1), position: SCNVector3(1.9, 1.35, -8.85)))

        for index in 0..<4 {
            let x = 2.35 - Float(index) * 0.22
            let y = 0.04 + Float(index) * 0.045
            root.addChildNode(box(width: 0.22, height: 0.09, length: 1.35, color: SceneColor(calibratedRed: 0.44, green: 0.44, blue: 0.42, alpha: 1), position: SCNVector3(x, y, -9.35)))
        }

        root.addChildNode(makeText("门厅", size: 0.075, color: SceneColor(calibratedWhite: 0.12, alpha: 1), position: SCNVector3(3.95, 1.95, -10.1)))
        return root
    }

    private func makeRestroomSign() -> SCNNode {
        let root = SCNNode()
        let tile = SceneColor(calibratedRed: 0.72, green: 0.82, blue: 0.84, alpha: 1)
        let partition = SceneColor(calibratedRed: 0.28, green: 0.42, blue: 0.55, alpha: 1)
        let porcelain = SceneColor(calibratedWhite: 0.93, alpha: 1)
        root.addChildNode(box(width: 1.18, height: 0.035, length: 1.18, color: SceneColor(calibratedRed: 0.58, green: 0.64, blue: 0.64, alpha: 1), position: SCNVector3(6.7, 0.0, 5.85)))
        ...
        root.addChildNode(box(width: 0.065, height: 0.24, length: 0.88, color: SceneColor(calibratedRed: 0.86, green: 0.94, blue: 1.0, alpha: 1), position: SCNVector3(6.14, 1.84, 5.85)))
        root.addChildNode(makeText("洗手间", size: 0.055, color: SceneColor(calibratedWhite: 0.05, alpha: 1), position: SCNVector3(6.09, 1.79, 5.58)))
        root.addChildNode(box(width: 0.018, height: 0.9, length: 0.1, color: SceneColor(calibratedWhite: 0.95, alpha: 1), position: SCNVector3(6.1, 0.82, 5.42)))
        root.addChildNode(box(width: 0.018, height: 0.9, length: 0.1, color: SceneColor(calibratedWhite: 0.95, alpha: 1), position: SCNVector3(6.1, 0.82, 6.28)))
        ...
        root.addChildNode(capsule(radius: 0.018, height: 0.18, color: SceneColor(calibratedWhite: 0.55, alpha: 1), position: SCNVector3(7.02, 0.86, 5.5), rotation: SCNVector4(1, 0, 0, Float.pi / 2)))
        ...
        root.addChildNode(box(width: 0.06, height: 1.1, length: 0.34, color: SceneColor(calibratedRed: 0.36, green: 0.5, blue: 0.62, alpha: 1), position: SCNVector3(6.46, 0.78, 6.12)))
        root.addChildNode(sphere(radius: 0.018, color: SceneColor(calibratedRed: 0.86, green: 0.68, blue: 0.28, alpha: 1), position: SCNVector3(6.42, 0.78, 5.99)))
        return root
    }

    private func makeCorridorNoticeArea() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.055, height: 0.7, length: 1.25, color: SceneColor(calibratedRed: 0.18, green: 0.32, blue: 0.22, alpha: 1), position: SCNVector3(6.56, 1.62, -2.2)))
        root.addChildNode(box(width: 0.06, height: 0.52, length: 1.04, color: SceneColor(calibratedRed: 0.9, green: 0.86, blue: 0.64, alpha: 1), position: SCNVector3(6.525, 1.62, -2.2)))
        root.addChildNode(makeText("年级通知", size: 0.052, color: SceneColor(calibratedWhite: 0.08, alpha: 1), position: SCNVector3(6.49, 1.82, -2.58)))
        root.addChildNode(makeText("保持安静", size: 0.048, color: SceneColor(calibratedWhite: 0.08, alpha: 1), position: SCNVector3(6.49, 1.56, -2.58)))
        root.addChildNode(box(width: 0.058, height: 0.34, length: 0.62, color: SceneColor(calibratedRed: 0.15, green: 0.48, blue: 0.34, alpha: 1), position: SCNVector3(6.53, 2.12, 1.8)))
        root.addChildNode(makeText("EXIT", size: 0.08, color: SceneColor(calibratedWhite: 0.96, alpha: 1), position: SCNVector3(6.49, 2.1, 1.55)))
        return root
    }

    private func makeWaterDispenser() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.34, height: 0.78, length: 0.32, color: SceneColor(calibratedWhite: 0.86, alpha: 1), position: SCNVector3(6.38, 0.42, 3.9)))
        root.addChildNode(box(width: 0.24, height: 0.24, length: 0.25, color: SceneColor(calibratedRed: 0.4, green: 0.72, blue: 0.95, alpha: 1), position: SCNVector3(6.36, 0.94, 3.9)))
        root.addChildNode(box(width: 0.035, height: 0.08, length: 0.045, color: SceneColor(calibratedRed: 0.84, green: 0.16, blue: 0.12, alpha: 1), position: SCNVector3(6.18, 0.58, 3.83)))
        root.addChildNode(box(width: 0.035, height: 0.08, length: 0.045, color: SceneColor(calibratedRed: 0.12, green: 0.32, blue: 0.72, alpha: 1), position: SCNVector3(6.18, 0.58, 3.97)))
        return root
    }
    ...
    private func makeTrashBin() -> SCNNode {
        ...
        body.firstMaterial = material(SceneColor(calibratedRed: 0.12, green: 0.42, blue: 0.28, alpha: 1))
        let bodyNode = SCNNode(geometry: body)
        bodyNode.position = SCNVector3(6.32, 0.23, -6.15)
        root.addChildNode(bodyNode)
        root.addChildNode(box(width: 0.28, height: 0.04, length: 0.28, color: SceneColor(calibratedRed: 0.08, green: 0.24, blue: 0.18, alpha: 1), position: SCNVector3(6.32, 0.48, -6.15)))
        return root
    }
    ...
    private func makeWindowLayer() -> SCNNode {
        ...
        outsideSkyNode.addChildNode(box(width: 0.03, height: 1.5, length: 6.4, color: SceneColor(calibratedRed: 0.45, green: 0.72, blue: 0.96, alpha: 1), position: SCNVector3(0, 0, 0)))
        outsideSkyNode.addChildNode(box(width: 0.035, height: 0.42, length: 0.7, color: SceneColor(calibratedRed: 0.18, green: 0.22, blue: 0.28, alpha: 1), position: SCNVector3(-0.008, -0.38, -2.1)))
        outsideSkyNode.addChildNode(box(width: 0.035, height: 0.62, length: 0.85, color: SceneColor(calibratedRed: 0.16, green: 0.2, blue: 0.26, alpha: 1), position: SCNVector3(-0.008, -0.28, -0.9)))
        outsideSkyNode.addChildNode(box(width: 0.035, height: 0.52, length: 0.72, color: SceneColor(calibratedRed: 0.14, green: 0.18, blue: 0.24, alpha: 1), position: SCNVector3(-0.008, -0.33, 0.55)))
        outsideSkyNode.addChildNode(box(width: 0.035, height: 0.68, length: 0.9, color: SceneColor(calibratedRed: 0.13, green: 0.17, blue: 0.22, alpha: 1), position: SCNVector3(-0.008, -0.24, 1.85)))
        outsideSkyNode.position = SCNVector3(-4.08, 1.75, -0.85)
        root.addChildNode(outsideSkyNode)

        outsideCloudNode.addChildNode(box(width: 0.032, height: 0.16, length: 0.9, color: SceneColor(calibratedWhite: 0.95, alpha: 0.72), position: SCNVector3(0, 0.28, -1.7)))
        outsideCloudNode.addChildNode(box(width: 0.032, height: 0.12, length: 1.15, color: SceneColor(calibratedWhite: 0.9, alpha: 0.66), position: SCNVector3(0, 0.1, 1.1)))
        outsideCloudNode.position = SCNVector3(-4.115, 1.78, -0.85)
        root.addChildNode(outsideCloudNode)

        outsideSunNode.addChildNode(sphere(radius: 0.13, color: SceneColor(calibratedRed: 1.0, green: 0.68, blue: 0.28, alpha: 1), position: SCNVector3(0, 0, 0)))
        outsideSunNode.position = SCNVector3(-4.13, 2.13, -2.2)
        root.addChildNode(outsideSunNode)

        outsideMoonNode.addChildNode(sphere(radius: 0.1, color: SceneColor(calibratedRed: 0.88, green: 0.92, blue: 1.0, alpha: 1), position: SCNVector3(0, 0, 0)))
        ...
        for z in stride(from: -2.9, through: 2.5, by: 0.45) {
            let rain = box(width: 0.018, height: 0.32, length: 0.012, color: SceneColor(calibratedRed: 0.68, green: 0.82, blue: 1.0, alpha: 0.58), position: SCNVector3(0, 0, Float(z)))
        ...
        outsideLampNode.addChildNode(box(width: 0.022, height: 0.72, length: 0.08, color: SceneColor(calibratedRed: 1.0, green: 0.72, blue: 0.28, alpha: 1), position: SCNVector3(0, 0, 0)))
        outsideLampNode.addChildNode(box(width: 0.024, height: 0.18, length: 0.9, color: SceneColor(calibratedRed: 1.0, green: 0.62, blue: 0.18, alpha: 1), position: SCNVector3(0, 0.28, 0)))
    ...
    private func glassPane(width: CGFloat, height: CGFloat, length: CGFloat, position: SCNVector3) -> SCNNode {
        ...
        glass.diffuse.contents = SceneColor(calibratedRed: 0.68, green: 0.88, blue: 1.0, alpha: 0.2)
        glass.specular.contents = SceneColor.white
        glass.emission.contents = SceneColor(calibratedRed: 0.18, green: 0.36, blue: 0.5, alpha: 0.05)
    ...
    private func windowFrame(at position: SCNVector3, height: CGFloat = 1.12, length: CGFloat = 1.46) -> SCNNode {
        ...
        let frameColor = SceneColor(calibratedRed: 0.72, green: 0.74, blue: 0.72, alpha: 1)
    ...
    private func makeDoors() -> SCNNode {
        let root = SCNNode()
        let doorColor = SceneColor(calibratedRed: 0.38, green: 0.25, blue: 0.15, alpha: 1)
        let trimColor = SceneColor(calibratedRed: 0.18, green: 0.12, blue: 0.08, alpha: 1)
        let knobColor = SceneColor(calibratedRed: 0.86, green: 0.68, blue: 0.28, alpha: 1)
    ...
    private func configureDoorLeaf(_ node: SCNNode, doorColor: SceneColor, knobColor: SceneColor, knobOffsetZ: Float) {
    ...
    private func addDoorFrame(to root: SCNNode, centerZ: Float, color: SceneColor) {
    ...
    private func makeInteriorDetails() -> SCNNode {
        let root = SCNNode()
        let seamColor = SceneColor(calibratedRed: 0.34, green: 0.28, blue: 0.22, alpha: 1)
        ...
        let baseboard = SceneColor(calibratedRed: 0.42, green: 0.33, blue: 0.24, alpha: 1)
    ...
    private func makeNoticeBoard() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.045, height: 0.92, length: 1.55, color: SceneColor(calibratedRed: 0.5, green: 0.28, blue: 0.16, alpha: 1), position: SCNVector3(-3.92, 1.72, 3.75)))
        root.addChildNode(box(width: 0.048, height: 0.72, length: 1.35, color: SceneColor(calibratedRed: 0.86, green: 0.78, blue: 0.52, alpha: 1), position: SCNVector3(-3.895, 1.72, 3.75)))
        for (index, z) in [3.28, 3.58, 3.92, 4.18].enumerated() {
            let paperColor = index % 2 == 0
                ? SceneColor(calibratedRed: 0.95, green: 0.95, blue: 0.9, alpha: 1)
                : SceneColor(calibratedRed: 0.88, green: 0.94, blue: 1.0, alpha: 1)
        ...
        root.addChildNode(makeText("值日表", size: 0.055, color: SceneColor(calibratedWhite: 0.08, alpha: 1), position: SCNVector3(-3.84, 2.14, 3.2)))
        return root
    }

    private func makeCleaningCorner() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.42, height: 0.32, length: 0.34, color: SceneColor(calibratedRed: 0.12, green: 0.32, blue: 0.36, alpha: 1), position: SCNVector3(-3.58, 0.16, 5.38)))
        root.addChildNode(box(width: 0.12, height: 0.72, length: 0.08, color: SceneColor(calibratedRed: 0.22, green: 0.36, blue: 0.22, alpha: 1), position: SCNVector3(-3.74, 0.62, 5.12)))
        root.addChildNode(box(width: 0.035, height: 1.18, length: 0.035, color: SceneColor(calibratedRed: 0.5, green: 0.34, blue: 0.2, alpha: 1), position: SCNVector3(-3.52, 0.74, 5.18)))
        root.addChildNode(box(width: 0.22, height: 0.1, length: 0.04, color: SceneColor(calibratedRed: 0.16, green: 0.38, blue: 0.56, alpha: 1), position: SCNVector3(-3.52, 1.32, 5.18)))
        return root
    }
    ...
    private func makeCeilingLights() -> SCNNode {
        ...
        for z in [-3.2, 0.0, 3.2] {
            let lightPanel = box(width: 1.35, height: 0.025, length: 0.22, color: SceneColor(calibratedWhite: 0.96, alpha: 1), position: SCNVector3(0, 3.47, Float(z)))
            lightPanel.geometry?.firstMaterial?.emission.contents = SceneColor(calibratedWhite: 0.72, alpha: 1)
            root.addChildNode(lightPanel)
            root.addChildNode(box(width: 1.46, height: 0.018, length: 0.03, color: SceneColor(calibratedWhite: 0.62, alpha: 1), position: SCNVector3(0, 3.45, Float(z) - 0.14)))
            root.addChildNode(box(width: 1.46, height: 0.018, length: 0.03, color: SceneColor(calibratedWhite: 0.62, alpha: 1), position: SCNVector3(0, 3.45, Float(z) + 0.14)))
        }
        return root
    }
    ...
    private func makeRightStorageCabinet() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.08, height: 1.55, length: 0.78, color: SceneColor(calibratedRed: 0.12, green: 0.23, blue: 0.3, alpha: 1), position: SCNVector3(3.92, 0.9, -2.35)))
        for (index, y) in [0.44, 0.78, 1.12, 1.46].enumerated() {
            root.addChildNode(box(width: 0.09, height: 0.035, length: 0.72, color: SceneColor(calibratedRed: 0.06, green: 0.1, blue: 0.13, alpha: 1), position: SCNVector3(3.86, Float(y), -2.35)))
            root.addChildNode(box(width: 0.11, height: 0.16, length: 0.11, color: index % 2 == 0 ? SceneColor(calibratedRed: 0.16, green: 0.48, blue: 0.72, alpha: 1) : SceneColor(calibratedRed: 0.58, green: 0.36, blue: 0.18, alpha: 1), position: SCNVector3(3.8, Float(y + 0.09), -2.58)))
            root.addChildNode(box(width: 0.11, height: 0.2, length: 0.1, color: index % 2 == 0 ? SceneColor(calibratedRed: 0.36, green: 0.22, blue: 0.62, alpha: 1) : SceneColor(calibratedRed: 0.12, green: 0.46, blue: 0.42, alpha: 1), position: SCNVector3(3.8, Float(y + 0.1), -2.22)))
        }
        root.addChildNode(makeText("备品柜", size: 0.045, color: SceneColor(calibratedWhite: 0.9, alpha: 1), position: SCNVector3(3.78, 1.72, -2.7)))
        return root
    }

    private func makeRearNewspaperWall() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 2.4, height: 0.78, length: 0.045, color: SceneColor(calibratedRed: 0.08, green: 0.16, blue: 0.14, alpha: 1), position: SCNVector3(-1.15, 1.78, 5.92)))
        root.addChildNode(makeText("班级黑板报", size: 0.078, color: SceneColor(calibratedWhite: 0.88, alpha: 1), position: SCNVector3(-2.18, 2.02, 5.86)))
        for (index, x) in [-1.92, -1.42, -0.92, -0.42].enumerated() {
            let paperColor = index % 2 == 0 ? SceneColor(calibratedRed: 0.92, green: 0.9, blue: 0.76, alpha: 1) : SceneColor(calibratedRed: 0.72, green: 0.86, blue: 0.92, alpha: 1)
            root.addChildNode(box(width: 0.34, height: 0.26, length: 0.02, color: paperColor, position: SCNVector3(Float(x), 1.68, 5.86)))
            root.addChildNode(box(width: 0.26, height: 0.035, length: 0.022, color: SceneColor(calibratedRed: 0.18, green: 0.36, blue: 0.62, alpha: 1), position: SCNVector3(Float(x), 1.78, 5.84)))
        }
        return root
    }
    ...
    private func makeAirConditioningDetails() -> SCNNode {
        let root = SCNNode()
        for z in [-3.15, 2.55] {
            root.addChildNode(box(width: 0.08, height: 0.26, length: 0.82, color: SceneColor(calibratedWhite: 0.86, alpha: 1), position: SCNVector3(3.93, 2.62, Float(z))))
            root.addChildNode(box(width: 0.085, height: 0.035, length: 0.66, color: SceneColor(calibratedWhite: 0.62, alpha: 1), position: SCNVector3(3.875, 2.52, Float(z))))
            root.addChildNode(box(width: 0.035, height: 0.38, length: 0.035, color: SceneColor(calibratedWhite: 0.72, alpha: 1), position: SCNVector3(3.91, 2.16, Float(z) + 0.34)))
        }
        root.addChildNode(box(width: 0.04, height: 0.42, length: 0.24, color: SceneColor(calibratedWhite: 0.82, alpha: 1), position: SCNVector3(3.92, 1.45, -3.18)))
        for (index, y) in [1.56, 1.44, 1.32].enumerated() {
            root.addChildNode(box(width: 0.045, height: 0.05, length: 0.05, color: index == 0 ? SceneColor(calibratedRed: 0.08, green: 0.44, blue: 0.48, alpha: 1) : SceneColor(calibratedRed: 0.22, green: 0.24, blue: 0.28, alpha: 1), position: SCNVector3(3.885, Float(y), -3.18)))
        }
        return root
    }
    ...
    private func makeDesk(at position: SCNVector3, isPlayer: Bool) -> SCNNode {
        let root = SCNNode()
        root.position = position
        let topColor = isPlayer ? SceneColor(calibratedRed: 0.74, green: 0.62, blue: 0.46, alpha: 1) : SceneColor(calibratedRed: 0.62, green: 0.48, blue: 0.34, alpha: 1)
        root.addChildNode(box(width: 0.74, height: 0.08, length: 0.53, color: topColor, position: SCNVector3(0, 0.72, 0)))
        root.addChildNode(box(width: 0.68, height: 0.035, length: 0.06, color: SceneColor(calibratedRed: 0.1, green: 0.1, blue: 0.11, alpha: 1), position: SCNVector3(0, 0.63, -0.24)))
        if isPlayer == false {
            root.addChildNode(makeDeskSupplies(seed: Int((position.x + 4) * 10 + (position.z + 6) * 7)))
        }
        for x in [-0.34, 0.34] {
            for z in [-0.2, 0.2] {
                root.addChildNode(box(width: 0.05, height: 0.68, length: 0.05, color: SceneColor.darkGray, position: SCNVector3(Float(x), 0.36, Float(z))))
            }
        }
        return root
    }
    ...
    private func makeDeskSupplies(seed: Int) -> SCNNode {
        let root = SCNNode()
        let bookColors = [
            SceneColor(calibratedRed: 0.18, green: 0.32, blue: 0.66, alpha: 1),
            SceneColor(calibratedRed: 0.6, green: 0.2, blue: 0.18, alpha: 1),
            SceneColor(calibratedRed: 0.22, green: 0.46, blue: 0.28, alpha: 1)
        ]
        ...
        if seed % 2 == 0 {
            root.addChildNode(box(width: 0.26, height: 0.012, length: 0.035, color: SceneColor(calibratedRed: 0.1, green: 0.14, blue: 0.32, alpha: 1), position: SCNVector3(0.16, 0.615, -0.18)))
        } else {
            root.addChildNode(box(width: 0.22, height: 0.04, length: 0.11, color: SceneColor(calibratedRed: 0.78, green: 0.68, blue: 0.42, alpha: 1), position: SCNVector3(0.18, 0.615, -0.18)))
        }
        return root
    }
    ...
    private func makeChair(at position: SCNVector3) -> SCNNode {
        let root = SCNNode()
        root.position = position
        root.addChildNode(box(width: 0.52, height: 0.06, length: 0.46, color: SceneColor(calibratedRed: 0.28, green: 0.24, blue: 0.22, alpha: 1), position: SCNVector3(0, 0.45, 0)))
        root.addChildNode(box(width: 0.52, height: 0.5, length: 0.06, color: SceneColor(calibratedRed: 0.25, green: 0.21, blue: 0.19, alpha: 1), position: SCNVector3(0, 0.75, 0.22)))
        let legColor = SceneColor(calibratedRed: 0.18, green: 0.18, blue: 0.17, alpha: 1)
    ...
    private func makeStudent(seed: Int, profile: ClassmateProfile?) -> SCNNode {
        ...
        let shirtColors = [
            SceneColor(calibratedRed: 0.18, green: 0.28, blue: 0.62, alpha: 1),
            SceneColor(calibratedRed: 0.88, green: 0.9, blue: 0.92, alpha: 1),
            SceneColor(calibratedRed: 0.32, green: 0.48, blue: 0.35, alpha: 1),
            SceneColor(calibratedRed: 0.55, green: 0.18, blue: 0.22, alpha: 1)
        ]
        ...
        let hair = sphere(radius: 0.152, color: SceneColor(calibratedWhite: 0.04 + CGFloat(seed % 3) * 0.04, alpha: 1), position: SCNVector3(0, 0.98, 0.02), scale: hairScale(seed: seed, profile: profile))
        hair.name = "hair"
        root.addChildNode(hair)

        if profile?.anxiety ?? 0 > 68 {
            root.addChildNode(box(width: 0.18, height: 0.01, length: 0.08, color: SceneColor(calibratedRed: 0.94, green: 0.86, blue: 0.3, alpha: 1), position: SCNVector3(-0.24, 0.2, -0.16)))
        }

        let phone = box(width: 0.12, height: 0.012, length: 0.2, color: SceneColor(calibratedRed: 0.04, green: 0.18, blue: 0.55, alpha: 1), position: SCNVector3(0.18, 0.2, -0.18))
        phone.name = "phone"
        phone.opacity = (profile?.rebelliousness ?? 0) > 72 ? 0.28 : 0
        root.addChildNode(phone)

        let paper = box(width: 0.22, height: 0.006, length: 0.16, color: SceneColor(calibratedWhite: 0.92, alpha: 1), position: SCNVector3(-0.12, 0.18, -0.2))
        paper.name = "paper"
        paper.opacity = (profile?.orderliness ?? 50) > 70 ? 0.82 : 0.55
        root.addChildNode(paper)

        if profile?.empathy ?? 0 > 72 {
            root.addChildNode(box(width: 0.08, height: 0.012, length: 0.12, color: SceneColor(calibratedRed: 0.88, green: 0.94, blue: 1.0, alpha: 1), position: SCNVector3(-0.22, 0.21, 0.02)))
        }
    ...
    private func makeTeacherGeometry() -> SCNNode {
        ...
        let head = sphere(radius: 0.15, color: SceneColor(calibratedRed: 0.9, green: 0.78, blue: 0.66, alpha: 1), position: SCNVector3(0, 1.28, 0))
        ...
        pressureLight.color = SceneColor(calibratedRed: 1.0, green: 0.48, blue: 0.24, alpha: 1)
    ...
    private func updateTeacherAttention(game: GameManager, teacherPosition: SCNVector3) {
        ...
        let color = rearDoor
            ? SceneColor(calibratedRed: 0.35, green: 0.62, blue: 1.0, alpha: 1)
            : SceneColor(calibratedRed: 1.0, green: 0.46, blue: 0.22, alpha: 1)
    ...
    private func makePlayerDeskProps() -> SCNNode {
        ...
        homeworkSheetNode.addChildNode(box(width: 0.58, height: 0.018, length: 0.48, color: SceneColor(calibratedWhite: 0.94, alpha: 1), position: SCNVector3(0, 0, 0)))
        homeworkSheetNode.addChildNode(box(width: 0.02, height: 0.004, length: 0.44, color: SceneColor(calibratedRed: 0.76, green: 0.1, blue: 0.12, alpha: 1), position: SCNVector3(-0.25, 0.014, 0)))
        for index in 0..<7 {
            let z = -0.18 + Float(index) * 0.06
            let lineWidth = CGFloat(index % 3 == 0 ? 0.4 : 0.46)
            homeworkSheetNode.addChildNode(box(width: lineWidth, height: 0.004, length: 0.012, color: SceneColor(calibratedRed: 0.72, green: 0.74, blue: 0.78, alpha: 1), position: SCNVector3(0.03, 0.016, z)))
        }
        ...
        homeworkProgressNode.addChildNode(box(width: 1, height: 0.01, length: 0.026, color: SceneColor(calibratedRed: 0.14, green: 0.42, blue: 0.95, alpha: 1), position: SCNVector3(0, 0, 0)))
        ...
        bladderIndicatorNode.addChildNode(box(width: 1, height: 0.012, length: 0.022, color: SceneColor(calibratedRed: 0.1, green: 0.65, blue: 0.72, alpha: 1), position: SCNVector3(0, 0, 0)))
        ...
        penNode.addChildNode(capsule(radius: 0.012, height: 0.38, color: SceneColor(calibratedRed: 0.08, green: 0.15, blue: 0.38, alpha: 1), position: SCNVector3(0.03, -0.01, -0.04), rotation: SCNVector4(0, 0, 1, Float.pi / 2)))
        penNode.addChildNode(sphere(radius: 0.018, color: SceneColor(calibratedRed: 0.02, green: 0.02, blue: 0.025, alpha: 1), position: SCNVector3(-0.17, -0.01, -0.04)))
        ...
        playerPhoneNode.addChildNode(box(width: 0.13, height: 0.018, length: 0.28, color: SceneColor(calibratedRed: 0.03, green: 0.035, blue: 0.045, alpha: 1), position: SCNVector3(0, 0, 0)))
        let phoneLight = SCNLight()
        phoneLight.type = .omni
        phoneLight.color = SceneColor(calibratedRed: 0.35, green: 0.55, blue: 1.0, alpha: 1)
        ...
        drawerShadowNode.addChildNode(box(width: 0.42, height: 0.012, length: 0.045, color: SceneColor(calibratedWhite: 0.04, alpha: 1), position: SCNVector3(0, 0, 0)))
        drawerShadowNode.position = SCNVector3(-0.6, 0.735, 1.73)
        drawerShadowNode.opacity = 0.18
        root.addChildNode(drawerShadowNode)

        drawerNode.addChildNode(box(width: 0.44, height: 0.05, length: 0.18, color: SceneColor(calibratedRed: 0.44, green: 0.32, blue: 0.22, alpha: 1), position: SCNVector3(0, 0, 0)))
        drawerNode.addChildNode(box(width: 0.1, height: 0.014, length: 0.014, color: SceneColor(calibratedRed: 0.78, green: 0.58, blue: 0.28, alpha: 1), position: SCNVector3(0, 0.01, 0.095)))
        drawerNode.position = SCNVector3(-0.6, 0.71, 1.66)
        drawerNode.opacity = 0.55
        root.addChildNode(drawerNode)

        snackWrapperNode.addChildNode(box(width: 0.18, height: 0.012, length: 0.12, color: SceneColor(calibratedRed: 0.95, green: 0.72, blue: 0.18, alpha: 1), position: SCNVector3(0, 0, 0)))
        snackWrapperNode.addChildNode(box(width: 0.16, height: 0.014, length: 0.035, color: SceneColor(calibratedRed: 0.82, green: 0.18, blue: 0.16, alpha: 1), position: SCNVector3(0, 0.008, 0.02)))
        ...
        leftLegNode.addChildNode(capsule(radius: 0.028, height: 0.48, color: SceneColor(calibratedRed: 0.12, green: 0.16, blue: 0.3, alpha: 1), position: SCNVector3(0, 0, 0), rotation: SCNVector4(1, 0, 0, Float.pi / 2)))
        leftLegNode.position = SCNVector3(-0.78, 0.43, 1.72)
        leftLegNode.eulerAngles.y = -0.12
        root.addChildNode(leftLegNode)

        rightLegNode.addChildNode(capsule(radius: 0.028, height: 0.48, color: SceneColor(calibratedRed: 0.12, green: 0.16, blue: 0.3, alpha: 1), position: SCNVector3(0, 0, 0), rotation: SCNVector4(1, 0, 0, Float.pi / 2)))
        rightLegNode.position = SCNVector3(-0.43, 0.43, 1.72)
        rightLegNode.eulerAngles.y = 0.12
        root.addChildNode(rightLegNode)

        seatTensionNode.addChildNode(box(width: 0.48, height: 0.012, length: 0.32, color: SceneColor(calibratedRed: 0.08, green: 0.18, blue: 0.2, alpha: 1), position: SCNVector3(0, 0, 0)))
    ...
    private func makeBlackboardDetails() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(makeText("55-SEAT CLASSROOM", size: 0.075, color: SceneColor(calibratedWhite: 0.82, alpha: 1), position: SCNVector3(-0.72, 2.43, -5.9)))
        root.addChildNode(makeText("晚自习", size: 0.22, color: SceneColor(calibratedWhite: 0.9, alpha: 1), position: SCNVector3(-1.75, 2.24, -5.9)))
        root.addChildNode(makeText("今日目标：完成作业  管理压力  允许求助", size: 0.075, color: SceneColor(calibratedWhite: 0.82, alpha: 1), position: SCNVector3(-1.78, 1.98, -5.9)))
        root.addChildNode(makeText("抬头会暴露，低头会失去信息。", size: 0.068, color: SceneColor(calibratedRed: 0.78, green: 0.92, blue: 0.78, alpha: 1), position: SCNVector3(-1.78, 1.8, -5.9)))
    ...
    private func makeSmartFrontWallDetails() -> SCNNode {
        ...
        root.addChildNode(box(width: 1.0, height: 0.5, length: 0.06, color: SceneColor(calibratedRed: 0.78, green: 0.71, blue: 0.58, alpha: 1), position: SCNVector3(0, 0.72, -4.62)))
        root.addChildNode(box(width: 0.56, height: 0.24, length: 0.065, color: SceneColor(calibratedRed: 0.04, green: 0.05, blue: 0.055, alpha: 1), position: SCNVector3(0, 0.72, -4.57)))
        root.addChildNode(box(width: 0.42, height: 0.15, length: 0.07, color: SceneColor(calibratedRed: 0.08, green: 0.18, blue: 0.22, alpha: 1), position: SCNVector3(0, 0.76, -4.535)))
        root.addChildNode(box(width: 0.74, height: 0.035, length: 0.05, color: SceneColor(calibratedRed: 0.18, green: 0.14, blue: 0.1, alpha: 1), position: SCNVector3(0, 1.0, -4.55)))

        let portColors = [
            SceneColor(calibratedRed: 0.1, green: 0.42, blue: 0.48, alpha: 1),
            SceneColor(calibratedRed: 0.12, green: 0.18, blue: 0.64, alpha: 1),
            SceneColor(calibratedWhite: 0.84, alpha: 1),
            SceneColor(calibratedRed: 0.42, green: 0.18, blue: 0.62, alpha: 1),
            SceneColor(calibratedRed: 0.78, green: 0.45, blue: 0.14, alpha: 1)
        ]
        ...
        root.addChildNode(box(width: 2.6, height: 0.035, length: 0.18, color: SceneColor(calibratedRed: 0.24, green: 0.22, blue: 0.2, alpha: 1), position: SCNVector3(0, 0.035, -4.92)))
        root.addChildNode(box(width: 0.08, height: 0.035, length: 1.5, color: SceneColor(calibratedRed: 0.18, green: 0.17, blue: 0.16, alpha: 1), position: SCNVector3(-1.2, 0.04, -4.25)))
        root.addChildNode(box(width: 0.08, height: 0.035, length: 1.5, color: SceneColor(calibratedRed: 0.18, green: 0.17, blue: 0.16, alpha: 1), position: SCNVector3(1.2, 0.04, -4.25)))
        return root
    }
    ...
    private func makeTouchPanel(x: Float, title: String) -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.82, height: 0.94, length: 0.055, color: SceneColor(calibratedRed: 0.02, green: 0.08, blue: 0.12, alpha: 1), position: SCNVector3(x, 1.9, -5.91)))
        root.childNodes.last?.geometry?.firstMaterial?.emission.contents = SceneColor(calibratedRed: 0.0, green: 0.18, blue: 0.26, alpha: 1)
        root.addChildNode(makeText(title, size: 0.065, color: SceneColor(calibratedRed: 0.72, green: 0.92, blue: 1.0, alpha: 1), position: SCNVector3(x - 0.32, 2.24, -5.86)))
        for index in 0..<4 {
            root.addChildNode(box(width: 0.46 - CGFloat(index) * 0.05, height: 0.035, length: 0.025, color: SceneColor(calibratedRed: 0.1 + CGFloat(index) * 0.08, green: 0.45, blue: 0.62, alpha: 1), position: SCNVector3(x - 0.12, 2.08 - Float(index) * 0.13, -5.85)))
        }
        return root
    }
    ...
    private func makeWallClock() -> SCNNode {
        ...
        face.firstMaterial = material(SceneColor(calibratedWhite: 0.92, alpha: 1))
        let faceNode = SCNNode(geometry: face)
        faceNode.position = SCNVector3(2.85, 2.62, -5.88)
        faceNode.eulerAngles.x = SceneScalar.pi / 2
        ...
        clockMinuteHandNode.addChildNode(box(width: 0.018, height: 0.2, length: 0.012, color: SceneColor(calibratedRed: 0.2, green: 0.2, blue: 0.22, alpha: 1), position: SCNVector3(0, 0.1, 0)))
        root.addChildNode(clockMinuteHandNode)
        root.addChildNode(sphere(radius: 0.025, color: SceneColor(calibratedWhite: 0.08, alpha: 1), position: SCNVector3(2.85, 2.62, -5.8)))
        return root
    }
    ...
    private func updateClock(game: GameManager) {
        ...
        let minuteAngle = -SceneScalar((totalMinutes / 60) * 2 * Double.pi)
        let hourAngle = -SceneScalar((totalMinutes / 720) * 2 * Double.pi)
    ...
    private func updateBlackboard(game: GameManager) {
        ...
        blackboardStatusNode.addChildNode(makeText(status, size: 0.07, color: SceneColor(calibratedRed: 0.94, green: 0.88, blue: 0.62, alpha: 1), position: SCNVector3Zero))
    }
    ...
    private func updateTimeAtmosphere(game: GameManager) {
        ...
        let skyColor: SceneColor
        let ambientColor: SceneColor
        let lampOpacity: CGFloat

        switch period {
        case .first:
            skyColor = SceneColor(calibratedRed: 0.58, green: 0.78, blue: 0.96, alpha: 1)
            ambientColor = SceneColor(calibratedRed: 0.78, green: 0.8, blue: 0.76, alpha: 1)
            lampOpacity = 0.12
        case .breakOne, .breakTwo:
            skyColor = SceneColor(calibratedRed: 0.72, green: 0.5, blue: 0.34, alpha: 1)
            ambientColor = SceneColor(calibratedRed: 0.74, green: 0.7, blue: 0.62, alpha: 1)
            lampOpacity = 0.38
        case .second:
            skyColor = SceneColor(calibratedRed: 0.12, green: 0.16, blue: 0.3, alpha: 1)
            ambientColor = SceneColor(calibratedRed: 0.58, green: 0.62, blue: 0.72, alpha: 1)
            lampOpacity = 0.56
        case .third:
            skyColor = SceneColor(calibratedRed: 0.015, green: 0.025, blue: 0.075, alpha: 1)
            ambientColor = SceneColor(calibratedRed: 0.5, green: 0.55, blue: 0.68, alpha: 1)
            lampOpacity = 0.78
        }

        ambientNode.light?.color = lightLevel < 0.6 ? SceneColor(calibratedRed: 0.18, green: 0.22, blue: 0.36, alpha: 1) : ambientColor
        ...
        outsideSunNode.position.y = SceneScalar(2.13 - progress * 0.9)
        outsideMoonNode.opacity = CGFloat(((progress - 0.42) * 1.9).clamped(to: 0...0.88))
        outsideMoonNode.position.y = SceneScalar(1.95 + progress * 0.38)
        ...
        outsideLampNode.childNodes.forEach {
            $0.geometry?.firstMaterial?.emission.contents = SceneColor(calibratedRed: 1.0, green: 0.58, blue: 0.18, alpha: lightLevel < 0.6 ? 0.85 : 0.38)
        }
    }
    ...
    private func updateDeskState(game: GameManager) {
        ...
        let stressTilt = SceneScalar(min(0.22, game.player.stress / 420))
        let bodyTension = SceneScalar(max(0, game.player.bladder - 52) / 48)
    ...
    private func makeText(_ string: String, size: CGFloat, color: SceneColor, position: SCNVector3) -> SCNNode {
        let geometry = SCNText(string: string, extrusionDepth: 0.002)
        geometry.font = SceneFont.systemFont(ofSize: size, weight: .semibold)
    ...
    private func applyStateAnimation(_ state: ClassmateState, to node: SCNNode) {
        ...
        case .usingPhone:
            ...
            node.childNode(withName: "phone", recursively: false)?.geometry?.firstMaterial?.emission.contents = SceneColor(calibratedRed: 0.05, green: 0.2, blue: 0.8, alpha: 1)
        case .anxious:
...
#if os(iOS)
        let isPhone = UIDevice.current.userInterfaceIdiom == .phone
        ambient.intensity = isPhone ? 280 : 230
        let lightXPositions: [Double] = isPhone ? [-2.5, 0, 2.5] : [-2.7, 0, 2.7]
        let lightZPositions: [Double] = isPhone ? [-2.4, 2.2] : [-3.6, -0.4, 2.8]
#else
        ambient.intensity = 230
        let lightXPositions: [Double] = [-2.7, 0, 2.7]
        let lightZPositions: [Double] = [-3.6, -0.4, 2.8]
#endif
        ambient.color = SceneColor(calibratedRed: 0.62, green: 0.67, blue: 0.78, alpha: 1)
        ambientNode.light = ambient
        root.addChildNode(ambientNode)

        for x in lightXPositions {
            for z in lightZPositions {
                let light = SCNLight()
                light.type = .omni
                light.intensity = 260
                light.color = SceneColor(calibratedRed: 1.0, green: 0.95, blue: 0.82, alpha: 1)
                light.attenuationStartDistance = 1.8
    ...
    private func makeCeilingFan(at position: SCNVector3) -> SCNNode {
        ...
        root.position = position
        root.opacity = 0.28
        root.addChildNode(capsule(radius: 0.018, height: 0.24, color: SceneColor(calibratedWhite: 0.42, alpha: 1), position: SCNVector3(0, 0.12, 0)))
        let hub = sphere(radius: 0.055, color: SceneColor(calibratedWhite: 0.34, alpha: 1), position: SCNVector3(0, 0, 0))
        root.addChildNode(hub)
        for angle in [0.0, Double.pi * 2 / 3, Double.pi * 4 / 3] {
            let blade = box(width: 0.62, height: 0.012, length: 0.075, color: SceneColor(calibratedWhite: 0.5, alpha: 1), position: SCNVector3(0.31, 0, 0))
            let bladeRoot = SCNNode()
            bladeRoot.eulerAngles.y = SceneScalar(angle)
    ...
    private func emissionColor(for state: ClassmateState) -> SceneColor {
        switch state {
        case .anxious:
            return SceneColor(calibratedRed: 0.18, green: 0.08, blue: 0.02, alpha: 1)
        case .crying:
            return SceneColor(calibratedRed: 0.26, green: 0.04, blue: 0.04, alpha: 1)
        case .offeringHelp, .covering, .lookingAtPlayer:
            return SceneColor(calibratedRed: 0.02, green: 0.14, blue: 0.12, alpha: 1)
        case .usingPhone:
            return SceneColor(calibratedRed: 0.02, green: 0.08, blue: 0.18, alpha: 1)
        default:
            return SceneColor.black
        }
    }

    private func box(width: CGFloat, height: CGFloat, length: CGFloat, color: SceneColor, position: SCNVector3) -> SCNNode {
    ...
    private func capsule(radius: CGFloat, height: CGFloat, color: SceneColor, position: SCNVector3, rotation: SCNVector4? = nil) -> SCNNode {
    ...
    private func sphere(radius: CGFloat, color: SceneColor, position: SCNVector3, scale: SCNVector3 = SCNVector3(1, 1, 1)) -> SCNNode {
    ...
    private func gazeCone() -> SCNNode {
        ...
        geometry.firstMaterial = material(SceneColor(calibratedRed: 1.0, green: 0.46, blue: 0.22, alpha: 0.18))
    ...
    private func makeStudentBody(uniformColor: SceneColor, seed: Int, profile: ClassmateProfile?) -> SCNNode {
        let root = SCNNode()
        let skin = skinColor(seed)
        let shirtLight = uniformColor.blended(withFraction: 0.28, of: .white) ?? uniformColor
        let pantsColor = SceneColor(calibratedRed: 0.1, green: 0.13, blue: 0.19, alpha: 1)
        let shoeColor = SceneColor(calibratedWhite: 0.045, alpha: 1)
        ...
        root.addChildNode(box(width: 0.055, height: 0.11, length: 0.018, color: SceneColor(calibratedWhite: 0.94, alpha: 1), position: SCNVector3(-0.035, 0.63, -0.091)))
        root.addChildNode(box(width: 0.055, height: 0.11, length: 0.018, color: SceneColor(calibratedWhite: 0.94, alpha: 1), position: SCNVector3(0.035, 0.63, -0.091)))
        ...
        if profile?.rebelliousness ?? 0 > 72 {
            root.addChildNode(box(width: 0.2, height: 0.025, length: 0.185, color: SceneColor(calibratedRed: 0.72, green: 0.18, blue: 0.2, alpha: 1), position: SCNVector3(0, 0.7, -0.015)))
        }
        return root
    }
    ...
    private func makeTeacherBody() -> SCNNode {
        let root = SCNNode()
        let skin = SceneColor(calibratedRed: 0.9, green: 0.78, blue: 0.66, alpha: 1)
        let jacket = SceneColor(calibratedRed: 0.14, green: 0.17, blue: 0.2, alpha: 1)
        let shirt = SceneColor(calibratedWhite: 0.9, alpha: 1)
        let pants = SceneColor(calibratedRed: 0.08, green: 0.09, blue: 0.11, alpha: 1)
        ...
        root.addChildNode(box(width: 0.06, height: 0.18, length: 0.024, color: SceneColor(calibratedRed: 0.42, green: 0.08, blue: 0.08, alpha: 1), position: SCNVector3(0, 0.92, -0.11)))
        ...
        root.addChildNode(capsule(radius: 0.045, height: 0.48, color: pants, position: SCNVector3(0.09, 0.34, 0)))
        root.addChildNode(box(width: 0.13, height: 0.05, length: 0.19, color: SceneColor(calibratedWhite: 0.035, alpha: 1), position: SCNVector3(-0.09, 0.06, -0.035)))
        root.addChildNode(box(width: 0.13, height: 0.05, length: 0.19, color: SceneColor(calibratedWhite: 0.035, alpha: 1), position: SCNVector3(0.09, 0.06, -0.035)))

        let notebook = box(width: 0.18, height: 0.025, length: 0.25, color: SceneColor(calibratedRed: 0.08, green: 0.16, blue: 0.34, alpha: 1), position: SCNVector3(-0.36, 0.65, -0.08))
    ...
    private func makeStudentFace(seed: Int, profile: ClassmateProfile?) -> SCNNode {
        let root = SCNNode()
        let eyeColor = SceneColor(calibratedWhite: 0.035, alpha: 1)
        let eyeWhite = SceneColor(calibratedWhite: 0.94, alpha: 1)
        let browColor = SceneColor(calibratedWhite: 0.035 + CGFloat(seed % 2) * 0.025, alpha: 1)
        let mouthColor = SceneColor(calibratedRed: 0.36, green: 0.12, blue: 0.11, alpha: 1)
        let noseColor = SceneColor(calibratedRed: 0.78, green: 0.55, blue: 0.43, alpha: 1)
        ...
        leftBrow.eulerAngles.z = SceneScalar.pi / 2 + (anxious ? -0.18 : (calm ? 0.06 : 0))
        rightBrow.eulerAngles.z = SceneScalar.pi / 2 + (anxious ? 0.18 : (calm ? -0.06 : 0))
        ...
        mouth.eulerAngles.z = SceneScalar.pi / 2 + (calm ? 0.04 : (anxious ? -0.04 : 0))
        root.addChildNode(mouth)

        if anxious {
            let shadowColor = SceneColor(calibratedWhite: 0.08, alpha: 0.72)
    ...
    private func makeTeacherFace() -> SCNNode {
        let root = SCNNode()
        let eyeColor = SceneColor(calibratedWhite: 0.025, alpha: 1)
        let eyeWhite = SceneColor(calibratedWhite: 0.94, alpha: 1)
        let browColor = SceneColor(calibratedWhite: 0.02, alpha: 1)
        ...
        leftBrow.eulerAngles.z = SceneScalar.pi / 2 - 0.08
        rightBrow.eulerAngles.z = SceneScalar.pi / 2 + 0.08
        root.addChildNode(leftBrow)
        root.addChildNode(rightBrow)

        let noseColor = SceneColor(calibratedRed: 0.75, green: 0.52, blue: 0.42, alpha: 1)
        ...
        root.addChildNode(capsule(radius: 0.006, height: 0.074, color: SceneColor(calibratedRed: 0.32, green: 0.1, blue: 0.1, alpha: 1), position: SCNVector3(0, -0.064, -0.157), rotation: SCNVector4(0, 0, 1, Float.pi / 2)))
    ...
    private func makeFaceGlasses() -> SCNNode {
        let root = SCNNode()
        let frame = SceneColor(calibratedWhite: 0.025, alpha: 1)
    ...
    private func makeGlasses() -> SCNNode {
        let root = SCNNode()
        root.addChildNode(box(width: 0.09, height: 0.012, length: 0.018, color: SceneColor(calibratedWhite: 0.04, alpha: 1), position: SCNVector3(-0.045, 0.805, -0.118)))
        root.addChildNode(box(width: 0.09, height: 0.012, length: 0.018, color: SceneColor(calibratedWhite: 0.04, alpha: 1), position: SCNVector3(0.045, 0.805, -0.118)))
        root.addChildNode(box(width: 0.028, height: 0.008, length: 0.014, color: SceneColor(calibratedWhite: 0.04, alpha: 1), position: SCNVector3(0, 0.805, -0.12)))
        return root
    }

    private func studentUniformColor(seed: Int, profile: ClassmateProfile?, fallback: SceneColor) -> SceneColor {
        guard let profile else { return fallback }
        if profile.rebelliousness > 72 {
            return SceneColor(calibratedRed: 0.46, green: 0.18, blue: 0.28, alpha: 1)
        }
        if profile.orderliness > 72 {
            return SceneColor(calibratedRed: 0.15, green: 0.2, blue: 0.46, alpha: 1)
        }
        if profile.empathy > 72 {
            return SceneColor(calibratedRed: 0.22, green: 0.42, blue: 0.38, alpha: 1)
        }
        if profile.anxiety > 68 {
            return SceneColor(calibratedRed: 0.36, green: 0.34, blue: 0.46, alpha: 1)
        }
        return fallback
    }
    ...
    private func material(_ color: SceneColor) -> SCNMaterial {
    ...
    private func skinColor(_ seed: Int) -> SceneColor {
        let colors = [
            SceneColor(calibratedRed: 0.95, green: 0.82, blue: 0.68, alpha: 1),
            SceneColor(calibratedRed: 0.86, green: 0.68, blue: 0.52, alpha: 1),
            SceneColor(calibratedRed: 0.76, green: 0.56, blue: 0.4, alpha: 1)
        ]
        return colors[seed % colors.count]
    }

    private var wallColor: SceneColor {
        SceneColor(calibratedRed: 0.82, green: 0.84, blue: 0.78, alpha: 1)
    }

    private var skinTone: SceneColor {
        SceneColor(calibratedRed: 0.92, green: 0.74, blue: 0.58, alpha: 1)
    }
}
